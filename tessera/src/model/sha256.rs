//! SHA-256 per FIPS 180-4, written out so the wasm build carries no digest dependency chain.
...
mod tests {
    ...
    fn lengths_around_the_padding_boundary() {
        // 55, 56, and 64 bytes exercise the one-block and two-block tails; checked against sha256sum.
        assert_eq!(
            hex(&digest(&[b'a'; 55])),
